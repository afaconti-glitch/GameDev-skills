...
- [`requirements-generator`](../requirements-generator/SKILL.md) — lightweight feature/system/tools intake.
- [`game-designer`](../game-designer/SKILL.md) or [`game-ux-designer`](../game-ux-designer/SKILL.md) — heavier discovery for design-led or UX-led work. Escalation only.
- [`shape-task`](../shape-task/SKILL.md) — brief → requirements + strategy + chunks (vertical slices).
...
- [`cleanup-verify`](../cleanup-verify/SKILL.md) — post-run build / perf / asset / save sweep.

Two documents define everything project-specific. Read them before classifying:

- [`project-adapter.md`](../../reference/project-adapter.md) — engine and version, build and test
  commands, gate chain, derived artefacts, perf budget, risky paths, host integrations. Resolved from
  `.claude/pipeline-adapter.md`, or discovered from the repository when absent.
- [`state-schema.md`](../../reference/state-schema.md) — the shared state contract, and how to degrade
  when state is unavailable.

Below, `<build>`, `<test>`, `<test.affected>`, `<cook>` and similar are placeholders for commands you
resolved from the adapter. They are never literal. `<cache>/` is the state directory the adapter
declares; it defaults to `.claude/cache/`.
...
5. **Phase M3 — Per-chunk loop.** For each chunk in order:
   - Run [`execute-chunk`](../execute-chunk/SKILL.md). Per-chunk validation: project / engine compiles, the most-affected automated test runs, and a targeted in-build smoke check is performed. Any save / network / cert / build-config gate the chunk touched runs in full.
...
You own `.claude/cache/pipeline.json` (see [`state-schema.md`](../../reference/state-schema.md)) for Medium and Large flows; Small flow does not touch the cache.
...
1. Read [`requirements-generator`](../requirements-generator/SKILL.md) and adopt that persona. Escalate to [`game-designer`](../game-designer/SKILL.md) or [`game-ux-designer`](../game-ux-designer/SKILL.md) only when the user explicitly asks for full design or UX discovery (mechanic spec from scratch, full HUD design, accessibility audit, etc.).
