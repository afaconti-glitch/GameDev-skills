...
Persist the shaped output into `.claude/cache/pipeline.json` so `execute-chunk`, `close-chunk`, and the TodoWrite hook can use it. See [`state-schema.md`](../../reference/state-schema.md) for the exact schema.
