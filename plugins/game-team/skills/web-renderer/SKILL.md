...
- `/three-js-best-practices` → Setup & Render Loop sections
...
Refer to: `/three-js-best-practices` → Draw Calls, Memory, and Debugging sections.
...
Refer to:
- `/threejs-shaders` — ShaderMaterial, RawShaderMaterial, uniform types, 6 GLSL patterns, onBeforeCompile
- `/three-js-best-practices` → Shaders and TSL sections
...
Refer to:
- `/threejs-loaders` — GLTFLoader, DRACO, KTX2, OBJ/FBX, LoadingManager, async patterns, caching
- `/threejs-textures` — colour spaces, wrapping, filtering, PBR texture set, KTX2, memory disposal
- `/three-js-best-practices` → Asset Loading section
...
Refer to: `/three-js-best-practices` → WebXR section.
...
Refer to:
- `/threejs-materials` — material type hierarchy and performance cost
- `/threejs-lighting` — shadow map budget and light count rules
- `/three-js-best-practices` → Mobile Optimisation section
...
Refer to:
- `/threejs-geometry` — InstancedMesh, geometry merging, InstancedBufferGeometry
- `/threejs-textures` → Texture Atlas section
- `/three-js-best-practices` → Draw Calls & Instancing section
...
Refer to:
- `/threejs-animation` — AnimationMixer, AnimationAction, skeletal, morph targets, blending, procedural spring physics
- `/threejs-fundamentals` → Clock usage
...
Refer to:
- `/threejs-interaction` — OrbitControls, PointerLockControls, raycasting, TransformControls, DragControls, keyboard/touch
...
Refer to:
- `/threejs-lighting` — 6 light types, shadow config, HDR/PMREM, 3-point studio setup, LightProbe
- `/threejs-materials` — material properties that interact with lighting (PBR, RectAreaLight requirements)
...
Refer to:
- `/threejs-postprocessing` — EffectComposer, pass catalogue, selective bloom, custom ShaderPass, resize handling
...
Refer to:
- `/threejs-fundamentals` → Proper Cleanup pattern
- `/threejs-textures` → Memory Management section
- `/three-js-best-practices` → Memory & Disposal section
